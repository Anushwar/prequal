...
train_df = pd.read_csv('/home/ubuntu/git_repo_SABRA/Data Mining/train.csv')
test_df = pd.read_csv('/home/ubuntu/git_repo_SABRA/Data Mining/test.csv')
...
for bar, cnt in zip(bars, vc.values):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height(),
            f'{(cnt/total)*100:.1f}%', ha='center', va='bottom', fontsize=11)
...
plot_cols = [c for c in numerical_cols if 'ohe_cols' not in globals() or c not in ohe_cols]

n = len(plot_cols); ncols = 3; nrows = int(np.ceil(n / ncols))
fig, axes = plt.subplots(nrows, ncols, figsize=(6*ncols, 4*nrows))
axes = np.array(axes).reshape(-1)

for idx, col in enumerate(plot_cols):
    axes[idx].hist(train_df[col].dropna(), bins=50, edgecolor='black', alpha=0.7)
    axes[idx].set_title(col, fontsize=11, fontweight='bold'); axes[idx].set_ylabel('Frequency')
...
cols_to_check = ['person_income', 'person_income_log',
                 'interest_to_income_ratio', 'interest_to_income_ratio_log',
                 'person_emp_length', 'person_emp_length_sqrt',]
...
plotting = [
    ...
    'debt_to_income_ratio','interest_to_income_ratio',
    'eff_cost_borrowing','cred_hist_x_int_rate','cred_hist_x_income'
]
...
numeric_df = (
    train_df
        ...
        .drop(columns=['loan_status'] + (ohe_cols if 'ohe_cols' in globals() else []), errors='ignore')
)

assert not set(numeric_df.columns) & set(ohe_cols), "numeric_df still contains OHE dummies."
assert 'loan_status' not in numeric_df.columns, "numeric_df must not include the target."
...
plt.scatter(pca_df[pca_df['loan_status']==0]['PC1'],
            pca_df[pca_df['loan_status']==0]['PC2'],
            label='Denied', alpha=0.6, c='#e74c3c')
plt.scatter(pca_df[pca_df['loan_status']==1]['PC1'],
            pca_df[pca_df['loan_status']==1]['PC2'],
            label='Approved', alpha=0.6, c='#2ecc71')
...
print(f"  y_train = train_df['loan_status']")
...
train_df.to_csv('/home/ubuntu/git_repo_SABRA/Data Mining/train_processed.csv', index=False)
test_df.to_csv('/home/ubuntu/git_repo_SABRA/Data Mining/test_processed.csv', index=False)
