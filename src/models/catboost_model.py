...
class LoanPredictionSystem:
    ...
    def __init__(self, random_state=42, use_pca=False, pca_var=0.95):
        ...
        self.pca = (
            PCA(n_components=pca_var, random_state=random_state) if use_pca else None
        )
        self.model = None
    ...
    def engineer_features(self, df):
        """
        Feature engineering – ratios, transformations, binary indicators
        """
        df = df.copy()
    ...
    def train(self, train_path="data/train.csv", use_smote=True):
        ...
        if self.use_pca:
            ...
            print(
                f"PCA applied → {self.pca.n_components_} components "
        ...
        if self.use_pca:
            feature_names = [
                f"PC{i+1}" for i in range(len(self.model.get_feature_importance()))
            ]
        else:
